...
class LayoutSVGGenerator:
    ...
    def _create_contiguous_shelf(
        ...
        logger.info(f"🏪 Creating shelf {shape_id} with {len(shape)} cells")
        ...
        logger.info(f"📐 Path data length: {len(path_data)} characters")
...
def generate_svg_from_h5(
    h5_filename: str, output_svg_path: str, include_metadata: bool = True
) -> Dict[str, Any]:
    """
    ...
    """
    generator = LayoutSVGGenerator()
    return generator.load_and_generate_svg(
        h5_filename, output_svg_path, include_metadata
    )
